...
struct NpuWrite32ToCertWrite32 : OpConversionPattern<AIEX::NpuWrite32Op> {
  ...
  matchAndRewrite(AIEX::NpuWrite32Op op, OpAdaptor adaptor,
    ...
    if (col && row) {
      ...
      uint32_t origAddress = op.getAddress();
    ...
    rewriter.replaceOpWithNewOp<AIEX::CertWrite32Op>(op, address,
                                                     op.getValue());
    return success();
  }
};
...
struct NpuMaskWrite32ToCertMaskWrite32
  ...
  matchAndRewrite(AIEX::NpuMaskWrite32Op op, OpAdaptor adaptor,
    ...
    rewriter.replaceOpWithNewOp<AIEX::CertMaskWrite32Op>(
        op, address, op.getMask(), op.getValue());
    return success();
  }
};
...
struct NpuSyncToCertWaitTCTS : OpConversionPattern<AIEX::NpuSyncOp> {
  ...
  matchAndRewrite(AIEX::NpuSyncOp op, OpAdaptor adaptor,
    ...
    // see table 3-23 in aie4 v1.5 spec and 3-1 in aie4z v0.9 spec
    std::vector<int> chan2actor_shim_s2mm = {0, 2, 3, 4};
    std::vector<int> chan2actor_shim_mm2s = {6, 7, 8, 9, 10, 11, 12, 13};
    // TODO: check for aie4z completness
    ...
    std::vector<int> *chan2actor =
        isS2MM ? &chan2actor_shim_s2mm : &chan2actor_shim_mm2s;
...
// Pattern to convert referenced devices' "configure" runtime sequences into
// cert.sections This runs early while they're still runtime_sequences, before
// RuntimeSequenceToCertJob
struct ConvertReferencedDevicesToSections : OpRewritePattern<AIE::DeviceOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(AIE::DeviceOp deviceOp,
                                PatternRewriter &rewriter) const override {
    // Find all cert.load_pdi operations in THIS device
    llvm::SmallVector<AIEX::CertLoadPdiOp, 4> loadPdiOps;
    deviceOp.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
      loadPdiOps.push_back(loadPdiOp);
    });

    if (loadPdiOps.empty())
      return failure(); // No load_pdi operations in this device

    // Get parent module to find referenced devices
    auto moduleOp = deviceOp->getParentOfType<ModuleOp>();
    if (!moduleOp)
      return failure();

    bool madeChanges = false;

    // For each load_pdi, convert the referenced device's "configure" sequence
    // to a section
    for (auto loadPdiOp : loadPdiOps) {
      StringRef refDeviceSymName = loadPdiOp.getSymbol().str();

      // Check if section already exists
      if (deviceOp.lookupSymbol(refDeviceSymName))
        continue; // Section already created
      ...
      for (auto &op : moduleOp.getBody()->getOperations()) {
        if (auto dev = dyn_cast<AIE::DeviceOp>(op)) {
          if (dev.getSymName() == refDeviceSymName) {
            refDevice = dev;
            break;
          }
        }
      ...
      if (!refDevice)
        continue; // Referenced device not found

      // Find the "configure" runtime sequence
      AIE::RuntimeSequenceOp configureSeq = nullptr;
      refDevice.walk([&](AIE::RuntimeSequenceOp runtimeSeq) {
        if (runtimeSeq.getSymName() == "configure") {
          configureSeq = runtimeSeq;
        }
      });

      if (!configureSeq)
        continue; // No configure sequence found

      // Create cert.section in THIS device
      rewriter.setInsertionPoint(deviceOp.getBody()->getTerminator());
      auto sectionOp = rewriter.create<AIEX::CertSectionOp>(
          refDevice.getLoc(), rewriter.getStringAttr(refDeviceSymName));
      ...
      auto pageOp = rewriter.create<AIEX::CertPageOp>(refDevice.getLoc());
      Block *pageBlock = new Block();
      pageOp.getBody().push_back(pageBlock);
      rewriter.setInsertionPointToStart(pageBlock);
      ...
      uint32_t maxJobId = 0;
      deviceOp.walk([&](AIEX::CertJobOp certJobOp) {
        maxJobId = std::max(maxJobId, certJobOp.getJobId());
      });
      ...
      // Create job in page
      auto jobOp =
          rewriter.create<AIEX::CertJobOp>(refDevice.getLoc(), sectionJobId);

      // Clone the configure runtime sequence body into the job
      IRMapping mapper;
      configureSeq.getRegion().cloneInto(&jobOp.getBody(), mapper);

      // Ensure terminators
      AIEX::CertJobOp::ensureTerminator(jobOp.getBody(), rewriter,
                                        refDevice.getLoc());
      AIEX::CertPageOp::ensureTerminator(pageOp.getBody(), rewriter,
                                         refDevice.getLoc());
...
struct NpuLoadPdiToCertLoadPdi : OpConversionPattern<AIEX::NpuLoadPdiOp> {
  ...
  matchAndRewrite(AIEX::NpuLoadPdiOp op, OpAdaptor adaptor,
    ...
    // Assign PDI ID - check if we already have a load_pdi for this device
    uint32_t pdiId = 0;
    bool foundExisting = false;
    parentDevice.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
      if (loadPdiOp.getSymbol().str() == deviceSymName) {
...
struct ConfigureOpToCertSection : OpRewritePattern<AIEX::ConfigureOp> {
  ...
  LogicalResult matchAndRewrite(AIEX::ConfigureOp op,
    ...
    if (parentDevice.lookupSymbol(deviceSymName)) {
      // Section already exists, just create load_pdi at call site
      // Find the PDI ID used for this section
      uint32_t pdiId = 0;
      bool foundExisting = false;
      parentDevice.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
        if (loadPdiOp.getSymbol().str() == deviceSymName) {
      ...
      // If we didn't find existing load_pdi for this section, assign new ID
      if (!foundExisting) {
        pdiId = 1;
        parentDevice.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
          pdiId = std::max(pdiId, loadPdiOp.getPdiId() + 1);
    ...
    // At the configure call site, replace with load_pdi
    // Use a unique PDI ID (for now, just use a simple counter based on existing
    // sections)
    uint32_t pdiId = 1;
    parentDevice.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
      pdiId = std::max(pdiId, loadPdiOp.getPdiId() + 1);
...
struct AIENpuToCertPass
  ...
  void runOnOperation() override {
    ...
    // This ensures Phase 0 can find referenced devices' "configure" sequences
    // before they get converted to cert.job
    ...
    // Phase 6: Remove absorbed devices (keep only main device)
  ...
  void processDevice(AIE::DeviceOp currentDevice) {
    ...
    // Phase 0: Identify and convert referenced "configure" sequences EARLY
    // Must happen before p0 converts RuntimeSequenceOp on other devices
    if (moduleOp) {

      // Collect npu.load_pdi operations (before they're converted in p2)
    ...
    // Phase 0b: Then convert ConfigureOps to cert.section + cert.load_pdi
    ...
    // Phase 4: Convert referenced devices to cert.sections
    ...
    // their cert jobs into sections in this device
    {
      ...
      currentDevice.walk([&](AIEX::CertLoadPdiOp loadPdiOp) {
        StringRef refSymName = loadPdiOp.getSymbol().str();
        if (!currentDevice.lookupSymbol(refSymName))
    ...
    // Phase 5: Clean up unused block arguments from cert.job operations
