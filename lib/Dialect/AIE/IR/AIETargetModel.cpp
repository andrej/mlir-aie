...
std::unique_ptr<RegisterDatabase>
AIE2PSTargetModel::loadRegisterDatabase() const {
  return RegisterDatabase::loadAIE2PS();
}
...
// switch, not through the shim_mux. We expose it in both switchbox and
// shim_mux connection counts so the pathfinder and column control overlay
// pass can route packet flows to it.
uint32_t
...
uint32_t
AIE2PSTargetModel::getNumDestShimMuxConnections(int col, int row,
                                                WireBundle bundle) const {
  if (isShimNOCTile(col, row) && bundle == WireBundle::Controller32)
    return 1;
  return AIE2TargetModel::getNumDestShimMuxConnections(col, row, bundle);
}

uint32_t
AIE2PSTargetModel::getNumSourceShimMuxConnections(int col, int row,
                                                  WireBundle bundle) const {
  if (isShimNOCTile(col, row) && bundle == WireBundle::Controller32)
    return 1;
  return AIE2TargetModel::getNumSourceShimMuxConnections(col, row, bundle);
}
