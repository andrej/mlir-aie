...
def my_matmul(M, K, N, m, k, n, n_aie_cols, dtype_in_str, dtype_out_str):
    ...
    # Microkernel MAC dimensions (used for dims_to_stream reshaping)
    # Using AIE2 dimensions for scalar kernel (doesn't affect correctness)
    r, s, t = 4, 4, 4
    ...
    # A distribution: use min(n_aie_cols, n_aie_rows) shim tiles for A
    ...
    # Tile layout for xcve3858:
    #   row 0: shim, rows 1-2: memtile, rows 3-6: core
    # Use row 1 for memtile (ignore row 2 for now, same as NPU2 pattern)
    core_row_offset = 3  # first core row
    ...
    for i in range(n_shim_mem_A):
        ...
        a_tmp_fifos = (
            ...
            .split(
                ...
                placement=Tile(i, 1),  # memtile row 1
            )
        )
    ...
    for col in range(n_aie_cols):
        ...
        B_l2l1_fifos[col] = (
            ...
            .forward(
                ...
                name=f"B_L2L1_{col}",
                dims_to_stream=dims_to_stream,
                placement=Tile(col, 1),  # memtile row 1
            )
        )
...
if __name__ == "__main__":
    ...
    p.add_argument("-N", type=int, default=1152)  # 32*36 = 1152, divisible by n*36
