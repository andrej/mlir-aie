...
//
// Minimal add-two design for xclbin-flow instruction buffer size limit test.
// Uses the dma_memcpy_nd API (no load_pdi) so the xclbin flow works without
// PDI patching in host code.
    ...
    aie.core(%t02) {
      ...
      scf.for %steps = %c0 to %c8 step %c1 {
        ...
        scf.for %i = %c0 to %c8 step %c1 {
          %0 = memref.load %elem0[%i] : memref<8xi32>
          %1 = arith.addi %0, %c2_i32 : i32
          memref.store %1, %elem1[%i] : memref<8xi32>
        }
