...
// --------------------------------------------------------------------------
// Address Patching (DDR_PATCH on the host side)
// --------------------------------------------------------------------------
...
test_utils::parse_address_patch_offsets_json(const std::string &json_path) {
  ...
  for (const auto &obj : objs) {
    ...
    info.offset_bytes =
        static_cast<uint32_t>(extract_json_int(obj, "offset_bytes"));
    info.arg_idx = static_cast<uint32_t>(extract_json_int(obj, "arg_idx"));
    info.arg_plus = static_cast<uint64_t>(extract_json_int(obj, "arg_plus"));
    info.addr = static_cast<uint64_t>(extract_json_int(obj, "addr"));
    results.push_back(info);
  }
...
// Walk the v0.1 transaction binary instruction by instruction.
// Returns the size (in uint32_t words) of the instruction at position i,
// or 0 if the opcode is unrecognized.
static uint32_t get_instr_size_words(const std::vector<uint32_t> &instr_v,
                                     size_t i) {
  if (i >= instr_v.size())
    return 0;
  ...
  default:
    if (opc >= 0x80) {
      // Custom ops: size at Word[1]
      if (i + 1 >= instr_v.size())
        return 0;
      uint32_t sz = instr_v[i + 1];
      if (sz == 0 || sz % 4 != 0)
        return 0;
      return sz / 4;
    }
...
void test_utils::patch_addresses(
    std::vector<uint32_t> &instr_v,
    const std::vector<AddressPatchInfo> &patches,
    const std::map<uint32_t, uint64_t> &arg_addresses, int verbosity) {
  if (patches.empty())
    return;

  // Detect and skip the 4-word transaction binary header.
  // The header's Word[0] has version/device info in the upper bytes,
  // whereas a real instruction has opcode in the low byte only.
  size_t i = 0;
  if (instr_v.size() >= 4 && (instr_v[0] & 0xFF) == 0 &&
      (instr_v[0] >> 8) != 0) {
    i = 4;
  }

  while (i < instr_v.size()) {
    uint32_t size_words = get_instr_size_words(instr_v, i);
    if (size_words == 0 || i + size_words > instr_v.size())
      break;

    uint32_t opc = instr_v[i] & 0xFF;
    if (opc == 0x01 /* XAIE_IO_BLOCKWRITE */ && size_words > 4) {
      uint32_t mmio_addr = instr_v[i + 2];
      uint32_t payload_bytes = (size_words - 4) * 4;

      for (size_t p = 0; p < patches.size(); ++p) {
        uint64_t paddr = patches[p].addr;
        if (paddr >= mmio_addr && paddr + 4 <= mmio_addr + payload_bytes) {
          auto it = arg_addresses.find(patches[p].arg_idx);
          if (it == arg_addresses.end())
            throw std::runtime_error(
                "patch_addresses: arg_idx " +
                std::to_string(patches[p].arg_idx) +
                " not found in arg_addresses");

          uint64_t full_addr = it->second + patches[p].arg_plus;
          uint32_t word_offset =
              static_cast<uint32_t>((paddr - mmio_addr) / 4);
          instr_v[i + 4 + word_offset] =
              static_cast<uint32_t>(full_addr & 0xFFFFFFFF);
          instr_v[i + 4 + word_offset + 1] =
              static_cast<uint32_t>(full_addr >> 32);

          if (verbosity >= 1)
            std::cout << "Patched address for arg_idx=" << patches[p].arg_idx
                      << " addr=0x" << std::hex << paddr << " -> 0x"
                      << full_addr << std::dec << "\n";
        }
      }
    }
