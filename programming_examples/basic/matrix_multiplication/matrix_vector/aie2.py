...
def my_matmul(M, K, n_cores, trace_sz):
    ...
    with mlir_mod_ctx() as ctx:
        ...
        def device_body():
            ...
            A1_L3L2 : list[Stream] = [None] * n_cores
            A2_L3L2 : list[Stream] = [None] * n_cores
            B_L3L2 : list[Stream] = [None] * n_cores
            A1_L2L1 : list[Stream] = [None] * n_cores
            A2_L2L1 : list[Stream] = [None] * n_cores
            ...
            for i in range(n_cores):
                ...
                A1_L3L2[i] = Stream(f"A1_L3L2_{i}", (ShimTile, 0), (MemTile, 0),  memref=memRef_inA_ty,  n_buffers=n_buffers_per_stream)
                A1_L3L2[i].generate()
                A2_L3L2[i] = Stream(f"A2_L3L2_{i}", (ShimTile, 0), (MemTile, 0),  memref=memRef_inA_ty,  n_buffers=n_buffers_per_stream)
                A2_L3L2[i].generate_dst_locks()

                B_L3L2[i] = Stream(f"B_L3L2_{i}", (ShimTile, 1), (MemTile, 1),  memref=memRef_inB_ty,  n_buffers=n_buffers_per_stream)
                B_L3L2[i].generate()

                A1_L2L1[i] = Stream(f"A1_L2L1_{i}", (MemTile, 0),  (CoreTile, 0), memref=memRef_inA_ty,  n_buffers=n_buffers_per_stream)
                A1_L2L1[i].generate()
                # Flow and locks for the second half of A (buffers of stream above are used for both halves)
                A2_L2L1[i] = Stream(f"A2_L2L1_{i}", (MemTile, 1), (CoreTile, 1), memref=memRef_inA_ty, n_buffers=n_buffers_per_stream)
                A2_L2L1[i].generate_src_locks()
                A2_L2L1[i].generate_dst_locks()
                A2_L2L1[i].generate_flow()
            ...
            for i in range(n_cores):
                ...
                with InsertionPoint(blocks[0]), Location.unknown():
                    MemTileDMAChain.append_start_op(DMAStartOp(DMAChannelDir.S2MM, channel_index=A1_L3L2[i].dstDMAChannel, dest=blocks[1]))
                for j in range(0, n_buffers_per_stream):
                    with InsertionPoint(blocks[2*j+1]), Location.unknown():
                        use_lock(A1_L3L2[i].dstProdLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L3L2[i].dstBuffers[j], offset=0, 
                               len=get_memref_size(A1_L3L2[i].dstMemref)//2)
                        use_lock(A1_L3L2[i].dstConsLock, LockAction.Release, value=1)
                        NextBDOp(blocks[2*j+2])
                    with InsertionPoint(blocks[2*j+2]), Location.unknown():
                        use_lock(A2_L3L2[i].dstProdLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L3L2[i].dstBuffers[j], offset=get_memref_size(A1_L3L2[i].dstMemref)//2, 
                               len=get_memref_size(A1_L3L2[i].dstMemref)//2)
                        use_lock(A2_L3L2[i].dstConsLock, LockAction.Release, value=1)
                ...
                blocks = MemTileDMAChain.append_blocks(n_dma_blocks_per_stream + n_buffers_per_stream)
                with InsertionPoint(blocks[0]), Location.unknown():
                    MemTileDMAChain.append_start_op(DMAStartOp(DMAChannelDir.MM2S, channel_index=A1_L2L1[i].srcDMAChannel, dest=blocks[1]))
                for j in range(0, n_buffers_per_stream):
                    with InsertionPoint(blocks[2*j+1]), Location.unknown():
                        use_lock(A1_L3L2[i].dstConsLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L3L2[i].dstBuffers[j], offset=0, len=get_memref_size(A1_L3L2[i].dstMemref)//2,
                               dimensions=[(k//2//2, 2), (m, k), (2, 1)] # transpose at 4-byte (2xbf16) granularity
                               )
                        use_lock(A1_L3L2[i].dstProdLock, LockAction.Release, value=1)
                        NextBDOp(blocks[2*j+2])
                    with InsertionPoint(blocks[2*j+2]), Location.unknown():
                ...
                blocks = MemTileDMAChain.append_blocks(n_dma_blocks_per_stream)
                with InsertionPoint(blocks[0]), Location.unknown():
                    MemTileDMAChain.append_start_op(DMAStartOp(DMAChannelDir.MM2S, channel_index=1, dest=blocks[1]))
                for j in range(0, n_buffers_per_stream):
                    with InsertionPoint(blocks[j+1]), Location.unknown():
                        use_lock(A2_L3L2[i].dstConsLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L3L2[i].dstBuffers[j], offset=k//2, len=get_memref_size(A1_L3L2[i].dstMemref)//2,
                               dimensions=[(k//2//2, 2), (m, k), (2, 1)] # transpose at 4-byte (2xbf16) granularity
                               )
                        use_lock(A2_L3L2[i].dstProdLock, LockAction.Release, value=1)
                ...
                with InsertionPoint(blocks[0]), Location.unknown():
                    CoreTileDMAChain.append_start_op(DMAStartOp(DMAChannelDir.S2MM, channel_index=A1_L2L1[i].dstDMAChannel, dest=blocks[1]))
                for j in range(0, n_buffers_per_stream):
                    with InsertionPoint(blocks[2*j+1]), Location.unknown():
                        use_lock(A1_L2L1[i].dstProdLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L2L1[i].dstBuffers[j], 
                               offset=0, 
                               len=get_memref_size(memRef_inA_ty)//2)
                        use_lock(A1_L2L1[i].dstConsLock, LockAction.Release, value=1)
                        NextBDOp(blocks[2*j+2])
                    with InsertionPoint(blocks[2*j+2]), Location.unknown():
                ...
                with InsertionPoint(blocks[0]), Location.unknown():
                    CoreTileDMAChain.append_start_op(DMAStartOp(DMAChannelDir.S2MM, channel_index=A2_L2L1[i].dstDMAChannel, dest=blocks[1]))
                for j in range(0, n_buffers_per_stream):
                    with InsertionPoint(blocks[j+1]), Location.unknown():
                        use_lock(A2_L2L1[i].dstProdLock, LockAction.AcquireGreaterEqual, value=1)
                        dma_bd(A1_L2L1[i].dstBuffers[j], 
                               offset=get_memref_size(memRef_inA_ty)//2, 
                               len=get_memref_size(memRef_inA_ty)//2)
                        use_lock(A2_L2L1[i].dstConsLock, LockAction.Release, value=1)
                        NextBDOp(blocks[j+2] if j+2<len(blocks) else blocks[1])
                        ...
                        for elem_out in C_L1L3[i].srcBuffers:
                            ...
                            for _ in for_(K // k // n_buffers_per_stream):
                                for j in range(n_buffers_per_stream):
                                    use_lock(A1_L2L1[i].dstConsLock, LockAction.AcquireGreaterEqual, value=1)
                                    use_lock(A2_L2L1[i].dstConsLock, LockAction.AcquireGreaterEqual, value=1)
                                    use_lock(B_L2L1[i].dstConsLock,  LockAction.AcquireGreaterEqual, value=1)
                                    elem_in_a = A1_L2L1[i].dstBuffers[j]
                                    elem_in_b = B_L2L1[i].dstBuffers[j]
                                    #call(matvec, [elem_in_a, elem_in_b, elem_out])
                                    call(passthrough_a, [elem_in_a, elem_in_b, elem_out])
                                    use_lock(A1_L2L1[i].dstProdLock, LockAction.Release, value=1)
                                    use_lock(A2_L2L1[i].dstProdLock, LockAction.Release, value=1)
            ...
            def sequence(A, B, C):
                ...
                for transfer_i in range(
                    ...
                    for i in range(n_cores):
                        ...
                        npu_dma_memcpy_nd(
                            metadata=f"{A1_L3L2[i].name}_src_buffer",
                            bd_id=1,
                            mem=A,
