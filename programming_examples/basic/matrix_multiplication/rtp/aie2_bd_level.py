...
def np_arr_t_to_mlir_t(t):
    return np_dtype_to_mlir_type(t.__args__[1].__args__[0])  

def get_memref_len_elems(memref):
    out = 1 
    for s in memref.__args__[0]:
        out *= s
    return out
...
def my_matmul(M, K, N, m, k, n, n_aie_cols, dtype_in_str, dtype_out_str):
    ...
    @device(dev)
    def device_body():
        A_l3_memref_ty = np.ndarray[(M * K,), np.dtype[dtype_in]]
        B_l3_memref_ty = np.ndarray[(K * N,), np.dtype[dtype_in]]
        C_l3_memref_ty = np.ndarray[(M * N,), np.dtype[dtype_out]]
        A_l2_memref_ty = np.ndarray[(m * k * n_A_tiles_per_shim,), np.dtype[dtype_in]]
        B_l2_memref_ty = np.ndarray[(k * n,), np.dtype[dtype_in]]
        C_l2_memref_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
        A_l1_memref_ty = np.ndarray[(m, k,), np.dtype[dtype_in]]
        B_l1_memref_ty = np.ndarray[(k, n,), np.dtype[dtype_in]]
        C_l1_memref_ty = np.ndarray[(m, n,), np.dtype[dtype_out]]
        ...
        for col in range(n_aie_cols):
            for row in range(n_aie_rows):
                ...
                # RTP index 1: K // k // 2
                rtp_bufs[row][col] = buffer(core_tiles[row][col], np.ndarray[(3,), np.dtype[np.int32]], f"rtp_{row}_{col}")
        ...
        for row in range(n_aie_rows):
            ...
            A_l2l1_fifos[row] = {
                "prod" : {
                    ...
                    "ping_buf": buffer(mem_tile, A_l2_memref_ty, name=f"A_L3L2_{row}_cons_buff_0"),
                    "pong_buf": buffer(mem_tile, A_l2_memref_ty, name=f"A_L3L2_{row}_cons_buff_1"),
                ...
                "cons" : [
                    {  
                        ...
                        "ping_buf": buffer(core_tiles[row][col], A_l1_memref_ty, name=f"A_L2L1_{row}_{col}_cons_buff_0"),
                        "pong_buf": buffer(core_tiles[row][col], A_l1_memref_ty, name=f"A_L2L1_{row}_{col}_cons_buff_1"),
        ...
        for col in range(n_aie_cols):
            ...
            A_l3l2_fifos[col] = {
                "prod" : {
                    ...
                    "shim_memref": memref_dialect.global_(sym_name=f"A_L3L2_{col}", sym_visibility="public", type_=np_arr_t_to_mlir_t(A_l3_memref_ty)),
        ...
        for col in range(n_aie_cols):
            ...
            B_l2l1_fifos[col] = {
                "prod" : {
                    ...
                    "ping_buf": buffer(mem_tile, B_l2_memref_ty, name=f"B_L3L2_{col}_cons_buff_0"),
                    "pong_buf": buffer(mem_tile, B_l2_memref_ty, name=f"B_L3L2_{col}_cons_buff_1"),
                ...
                "cons" : [
                    {  
                        ...
                        "ping_buf": buffer(core_tiles[row][col], B_l1_memref_ty, name=f"B_L2L1_{col}_{row}_cons_buff_0"),
                        "pong_buf": buffer(core_tiles[row][col], B_l1_memref_ty, name=f"B_L2L1_{col}_{row}_cons_buff_1"),
        ...
        for col in range(n_aie_cols):
            ...
            B_l3l2_fifos[col] = {
                "prod" : {
                    ...
                    "shim_memref": memref_dialect.global_(sym_name=f"B_L3L2_{col}", sym_visibility="public", type_=np_arr_t_to_mlir_t(B_l3_memref_ty)),
        ...
        for col in range(n_aie_cols):
            for row in range(n_aie_rows):
                C_l1l2_fifos[row][col] = {
                    "prod" : {
                        "endpoint": (core_tiles[row][col], WireBundle.DMA, 0),
                        "ping_buf": buffer(core_tiles[row][col], C_l1_memref_ty, name=f"C_L1L2_{col}_{row}_buff_0"),
                        "pong_buf": buffer(core_tiles[row][col], C_l1_memref_ty, name=f"C_L1L2_{col}_{row}_buff_1"),
        ...
        for col in range(n_aie_cols):
            C_l2l3_fifos[col] = {
                "prod" : {
                    ...
                    "ping_buf": buffer(mem_tiles[col], C_l2_memref_ty, name=f"C_L2L3_{col}_buff_0"),
                    "pong_buf": buffer(mem_tiles[col], C_l2_memref_ty, name=f"C_L2L3_{col}_buff_1"),
                ...
                "cons" : {  
                    ...
                    "shim_memref": memref_dialect.global_(sym_name=f"C_L2L3_{col}", sym_visibility="public", type_=np_arr_t_to_mlir_t(C_l3_memref_ty)),
                ...
                def core_body():
                    ...
                    with InsertionPoint(run_loop.body):
                        ...
                        with InsertionPoint(tile_loop.body):
                            ...
                            ifop = IfOp(c_pp_cond, [np_arr_t_to_mlir_t(C_l1_memref_ty)], hasElse=True)
