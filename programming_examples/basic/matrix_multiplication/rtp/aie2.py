    ...
    def device_body():
        ...
        for col in range(n_aie_cols):
            for row in range(n_aie_rows):
                ...
                # RTP index 1: K // k // 2
                rtp_bufs[row][col] = buffer(core_tiles[row][col], rtp_ty, f"rtp_{row}_{col}")
            ...
            for col in range(n_aie_cols):
                ...
                def core_body():
                    for _ in for_(0xFFFFFFFF):
                        ...
                        await_rtp(rtp_bufs[row][col])
                        rtp_K_div_k_i32 = get_volatile_rtp(rtp_bufs[row][col], 1)
                        rtp_K_div_k = index_dialect.castu(T.index(), rtp_K_div_k_i32)
                        rtp_n_tiles_per_core_i32 = get_volatile_rtp(rtp_bufs[row][col], 2)
                        rtp_n_tiles_per_core = index_dialect.castu(T.index(), rtp_n_tiles_per_core_i32)

                        for i in for_(rtp_n_tiles_per_core):

                            elem_out = C_l1l2_fifos[row][col].acquire(
                                ObjectFifoPort.Produce, 1
                            )
                            zero(elem_out)
                            for j in for_(rtp_K_div_k):
        ...
        def sequence(A, B, C):
            ...
            for row in range(n_aie_rows):
                for col in range(n_aie_cols):
                    ...
                    npu_rtp_write(sym_ref, 1, rtp_K_div_k)
