...
module {
  aie.device(npu2) {
    ...
    aie.core(%t02) {
      %c_intmax = arith.constant 0xFFFFFFFF : index
      %c0 = arith.constant 0 : index
      %c1 = arith.constant 1 : index
      %c8 = arith.constant 8 : index
      %c1_32 = arith.constant 41 : i32
  
      scf.for %steps = %c0 to %c_intmax step %c1 {
    ...
    aiex.runtime_sequence(%in : memref<1048576xi32>, %out : memref<1048576xi32>) {
      %c0 = arith.constant 0 : i64
      %c1 = arith.constant 1 : i64
      %c1048576 = arith.constant 1048576 : i64
      aiex.npu.dma_memcpy_nd (%out[%c0,%c0,%c0,%c0][%c1,%c1,%c1,%c1048576][%c0,%c0,%c0, %c1]) { metadata = @objFifo_out0, id = 1 : i64 } : memref<1048576xi32>
      aiex.npu.dma_memcpy_nd (%in[%c0,%c0,%c0,%c0][%c1,%c1,%c1,%c1048576][%c0,%c0,%c0, %c1]) { metadata = @objFifo_in0, id = 0 : i64, issue_token = true } : memref<1048576xi32>
      aiex.npu.dma_wait { symbol = @objFifo_out0 }
      aiex.npu.preempt { level = 1 : ui8 }
      aiex.npu.dma_memcpy_nd (%out[%c0,%c0,%c0,%c0][%c1,%c1,%c1,%c1048576][%c0,%c0,%c0, %c1]) { metadata = @objFifo_out0, id = 1 : i64 } : memref<1048576xi32>
      aiex.npu.dma_memcpy_nd (%in[%c0,%c0,%c0,%c0][%c1,%c1,%c1,%c1048576][%c0,%c0,%c0, %c1]) { metadata = @objFifo_in0, id = 0 : i64, issue_token = true } : memref<1048576xi32>
      aiex.npu.dma_wait { symbol = @objFifo_out0 }
    }
