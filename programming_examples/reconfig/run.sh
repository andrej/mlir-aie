...
if [[ $MODE -eq 2 ]] then
    ...
    for i in $(seq $ITERS); do
        ./build/test ./build/empty.xclbin ./build/config_add_two_insts.bin ./build/empty.xclbin ./build/run_add_two_insts.bin ./build/empty.xclbin ./build/config_subtract_three_insts.bin ./build/empty.xclbin ./build/run_subtract_three_insts.bin
    done
fi
