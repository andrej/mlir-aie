...
int main(int argc, const char *argv[]) {
  ...
  for (const auto &binfo : buffers_config) {
    int group_id = (binfo.name == "weights_1" || binfo.name == "weights_2" ||
                    binfo.name == "input")
                       ? 3
                       : 4;
    int kernel_idx = (binfo.name == "weights_3")      ? 3
                     : (binfo.name == "left_swished") ? 1
                     : (binfo.name == "intermediate") ? 2
                                                      : 0;
  ...
  for (int test_case = 0; test_case < NUM_TEST_CASES; test_case++) {
    ...
    for (const auto &op : runlist_config) {
      ...
      if (op.buffer_names.size() == 3) {
        auto run = kernels[kidx](
            opcode, bo_instrs[kidx], instr_data[kidx].size(),
            buffers[op.buffer_names[0]], buffers[op.buffer_names[1]],
