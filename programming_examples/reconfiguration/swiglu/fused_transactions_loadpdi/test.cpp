...
int main(int argc, const char *argv[]) {
  ...
  for (int test_case = 0; test_case < NUM_TEST_CASES; test_case++) {
    ...
    std::memcpy(buffer_ptrs["input"], input_data.data(),
                std::min(input_data.size(), buffers_config[0].size) *
                    sizeof(dtype));
    std::memcpy(buffer_ptrs["weights_1"], weights_1_data.data(),
                std::min(weights_1_data.size(), buffers_config[1].size) *
                    sizeof(dtype));
    std::memcpy(buffer_ptrs["weights_2"], weights_2_data.data(),
                std::min(weights_2_data.size(), buffers_config[2].size) *
                    sizeof(dtype));
    std::memcpy(buffer_ptrs["weights_3"], weights_3_data.data(),
                std::min(weights_3_data.size(), buffers_config[3].size) *
                    sizeof(dtype));
  ...
  for (int i = 0; i < NUM_ITERATIONS; i++) {
    std::cout << "fused_transactions_loadpdi," << i << "," << iteration_times[i]
              << std::endl;
  }
