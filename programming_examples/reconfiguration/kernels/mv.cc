...
void matvec_scalar(uint32_t m, uint32_t k, uint32_t row_offset, bfloat16 *a,
...
template <uint32_t r>
void matvec_vectorized(uint32_t m, uint32_t k, uint32_t row_offset,
  ...
  for (; c < c_end; c++) {
    ...
    // they do assume that k is at least two. This assumption should hold for
    // any useful use of this function; if k were one, this would be a simple
...
void matvec_scalar_bf16_bf16(uint32_t m, uint32_t k, uint32_t row_offset,
                             bfloat16 *a_in, bfloat16 *b_in, bfloat16 *c_out) {
  matvec_scalar(m, k, row_offset, a_in, b_in, c_out);
}

void matvec_vectorized_bf16_bf16(uint32_t m, uint32_t k, uint32_t row_offset,
                                 bfloat16 *a_in, bfloat16 *b_in,
                                 bfloat16 *c_out) {
